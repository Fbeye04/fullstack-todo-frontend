export default function TaskItem({ task, onToggle, onDelete, onEdit }) {
  return (
    <div className='w-full flex justify-between gap-4 bg-white border-2 border-black rounded-lg p-4 lg:p-6 group'>
      <div className='flex items-center gap-4'>
        <input
          type='checkbox'
          checked={task.isDone}
          onChange={() => onToggle(task.id, !task.isDone)}
          className='size-8 md:size-9 flex-none'
        />
        <span
          className={`text-2xl md:text-3xl flex-grow ${
